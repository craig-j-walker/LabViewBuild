...
async function run() {
    try {
        ...
        const lvpath = <string>tl.getInput('lvpath');
        ...
        for (let i = 1; i <= retries; i++) {
            try {
                ...
                var arg : string[] = ["-LabVIEWPath", lvpath, "-OperationName", "ExecuteBuildSpec", "-ProjectPath", projectfile, "-TargetName", targetName, "-BuildSpecName", buildSpecName, "-PortNumber", portNumber.toString(), "-LogFilePath", logfilePath];
